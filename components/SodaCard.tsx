  ...
  return (
    <Link href={`/soda/${soda.id}`} className="group block">
      ...
      <div className="mt-2 min-w-0">
        ...
        <div className="truncate text-xs font-semibold text-[#CBBCC2]">{subtitle}</div>
        ...
        <div className="mt-2 flex flex-wrap gap-1">
          ...
          {flavorTags.map((tag) => (
            <span key={tag} className="rounded bg-[#362B32] px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-[#CBBCC2]">
              {tag}
            </span>
          ))}
        </div>
